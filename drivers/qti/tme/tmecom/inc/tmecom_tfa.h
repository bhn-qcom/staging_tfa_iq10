...
/**
 ...
 /**
   ...
   */
typedef struct tmecom_client_info {
	const char *channel_name;
} tmecom_client_info_t;
...
/**
  ...
  *
  * @param [in]  client_ptr  Pointer to a unique opaque handle identifying the
...
/**
  * Send a request to another EE (Execution Environment) using the QMP
  * interface.
  *
  * @param [in]     client_ptr   Pointer to tmecom client.
  * @param [in]     req_ptr      Pointer to a buffer to send to another EE.
  * @param [in]     req_size     Size of the buffer to send to another EE (in
  *                              bytes).
  * @param [out]    resp_ptr     Pointer to a buffer to receive a response from
  *                              another EE.
  * @param [in/out] resp_size    Size of the buffer to receive a response from
  *                              another EE (in bytes).
  * @param [in]     timeout_msec Timeout in msec for receiving a response.
  *
  * @return @c 0 if successfully handled, error code otherwise
  *
  * @note The function @c tmecom_client_send_message_sync() is a blocking
  *       function and will not return unless there is either a timeout or the
  *       other EE returns with a response.
  */
