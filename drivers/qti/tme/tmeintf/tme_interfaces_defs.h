...
typedef uint64_t tme_com_addr_t;
#else   /* FEATURE_64_BIT_HSDMA */
typedef uint32_t tme_com_addr_t;
 ...
 */
typedef PACKED_STRUCT tme_sha_req {
	uint32_t       algorithm; /**< SHA algorithm */
	tme_com_addr_t data;      /**< Input data address */
	uint32_t       data_size; /**< Input data size */
	uint32_t       key_id;    /**< Key identifier (applicable for HMAC-SHA) */
} tme_sha_req_t;
 ...
 * sequencer.
 */
typedef struct tme_extended_error_info {
 ...
 *
 * Field order copied verbatim from TME FW's own tme_message_types.h - status
 ...
 * tme_message_types.h.  TME FW requires the whole struct: its handler rejects
 ...
 * Values copied verbatim from TME FW's tme_message_types.h - do not renumber.
 */
typedef enum
{
	QFPROM_BIST_CTRL = 1,
 ...
 */
typedef PACKED_STRUCT tme_sha_rsp {
	tme_extended_error_info_t info;
	uint8_t output[TME_HA_SHA512_SIZE];
	uint32_t output_len;
} tme_sha_rsp_t;
