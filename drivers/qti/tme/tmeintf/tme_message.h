 ...
 * NOTE: Does not currently check that the data is in the shared TME DDR
 * region. */
...
#define MAX_CBOR_REQ_LENGTH \
	(TMECOM_MAX_REQUEST_SIZE - sizeof(tmecom_msg_hdr_t))
#define MAX_CBOR_RSP_LENGTH \
	(TMECOM_MAX_RESPONSE_SIZE - sizeof(tmecom_msg_hdr_t))

/*
 * get_encoded_number_size() - Return the number of bytes CBOR needs to encode
 * @number.
 */
size_t get_encoded_number_size(uint32_t number);

/*
 * get_max_request_payload() - Return the maximum raw payload that fits in a
 * CBOR request to TME.
 *
...
/*
 * get_max_response_payload() - Return the maximum raw payload that fits in a
 * CBOR response from TME.
 *
 * Accounts for the CBOR tag and length overhead around the payload.
 */
...
/*
 * encode_message() - CBOR-encode @message_buf into @encoded_buf, tagged with
 * @tag.
 *
 * @param [in]     tag         CBOR tag identifying the TME operation.
 * @param [in]     message_buf  Raw message bytes to encode.
 * @param [in/out] encoded_buf  Output buffer; on return, len holds encoded
 *                              size.
 *
...
/*
 * decode_message() - CBOR-decode @encoded_buf into @message_buf, verifying
 * @tag.
 *
 * @param [in]     tag         Expected CBOR tag.
 * @param [in]     encoded_buf  CBOR-encoded input.
 * @param [in/out] message_buf  Output buffer; on return, len holds decoded
 *                              size.
 *
...
/*
 * transceive_message() - CBOR-encode a request, send it to TME, then
 * CBOR-decode
 ...
 *
 * @param [out] error_info  Destination for the error information (must not be
 *                          NULL).
 * @param [in]  result      Source error information to copy.
 *
 * @return 0 if all error fields are zero, -EIO otherwise.
 *         Returns -EINVAL if @c error_info is NULL.
 */
