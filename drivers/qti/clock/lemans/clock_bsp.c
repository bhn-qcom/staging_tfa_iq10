 ...
 *
 * Clock BSP data for lemans (qcs9075). Only the data the boot flow uses is
...
static struct clock_group clock_groups[CLOCK_GROUP_TOTAL] = {
	[CLOCK_GROUP_INIT] = {
		.clocks = (struct clock_clk_desc[]) {
			...
			{ GCC_TURING_1_Q6_TBU2_CBCR, .part = CHIPINFO_PART_NSP, .part_idx = 1 },
			{ GCC_TURING_1_Q6_TBU3_CBCR, .part = CHIPINFO_PART_NSP, .part_idx = 1 },
			{ GCC_TURING_0_Q6_TBU0_CBCR, .part = CHIPINFO_PART_NSP, .part_idx = 0 },
			{ GCC_TURING_0_Q6_TBU1_CBCR, .part = CHIPINFO_PART_NSP, .part_idx = 0 },
